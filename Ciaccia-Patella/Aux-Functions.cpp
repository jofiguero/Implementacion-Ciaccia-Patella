...
vector<Point> setAleatorio(vector<Point> conjunto, int k) {
    vector<Point> muestra;
    srand(time(0)); // Inicializa la semilla para la generación de números aleatorios

    // Mientras la muestra no tenga el tamaño deseado
    while (muestra.size() < k) {
        // Selecciona un elemento aleatorio del conjunto
        auto it = next(begin(conjunto), rand() % conjunto.size());
        Point elemento = *it;
        // Agrega el elemento a la muestra
        muestra.push_back(elemento);
    }
...
int alturaMinima(vector<AS*> ASs) {
    ...
    for (AS* as : ASs) {
        ...
        printf("la altura calculada es %d\n",alturaArbol);
...
void buscarSample(Mtree *tree, Point q,  Entry **retorno){
    set<Entry> *entradas = &tree->root->entries;
    if (isLeaf(*tree)){
        for (Entry entrada: *entradas) {
            if (entrada.p ==q){
                *retorno = &entrada; 
        ...
        for (Entry entrada: *entradas) {
            if (dist(entrada.p, q) <= entrada.cr){
                buscarSample(entrada.a, q, retorno);
            }
        }
    };
    return;
...
void SubarbolesValidos(Mtree *ptr_arbol, Point sample, vector<AS*>& contenedor,vector<Point>& F,int h){
    if(ptr_arbol ==NULL || altura(ptr_arbol)<h){
        ...
        if(altura(ptr_arbol)==h){
            ...
            AS* newas = new AS(sample, ptr_arbol);
...
/*Esta funcion recibe un puntero hacia el arbol inferior (Tinf), el sample (sample) y el arbol en el
que se debe insertar Tinf. Para esto debe buscar el sample en Tsup, y colgarle el arbol en la */
void unirArboles(Mtree *Tinf, Point sample, Mtree *Tsup) {
    Entry *puntero= (Entry *) malloc(sizeof(void *));
    buscarSample(Tsup,sample,&puntero);
    puntero->a = Tinf;
    return;
};
